...
var macroPatternMatches = function(tree1, tree2) {
  var i;
  var args = [];
  var lastArg;
  var el;

  for (i = 0; i < tree1.contents.length; i++) {
    el = tree1.contents[i];

    if (el.type === 'macro-pattern') {
      if (lastArg) {
        lastArg.type = 'expansion';
      }
    } else {
      var arg = {
        type: 'regular',
  ...
  for (i = 0; i < args.length; i++) {
    if (args[i].type !== 'expansion') {
      minArgs++;
    }
  }

  return tree2.contents.length >= minArgs;
};

var macroMatchesTree = function(tree1, tree2) {
  ...
  if (isList(tree1) && isList(tree2)) {
    if (noMacroPattern(tree1) && noMacroPattern(tree2)) {
      if (tree1.contents.length !== tree2.contents.length) {
        return false;
      }
