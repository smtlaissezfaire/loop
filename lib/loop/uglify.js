...
var handleBinary = function(f, args) {
  if (args.length !== 2) {
...
var handleMultiBinary = function(f, args) {
  if (args.length < 2) {
    throw new Error("Multi/Binary expression requires at least two arguments");
  }

  if (args.length === 2) {
    return handleBinary(f, args);
  } else {
    var last = args.pop();
    var subBinaryExpression = {
      type: 'funcall',
      function: {
        type: 'id',
        contents: '+'
    ...
    return handleBinary(f, [subBinaryExpression, last]);
  }
};

var handleUniary = function(f, args) {
  if (args.length !== 1) {
...
var BINARY_OR_UNIARY_OPERATORS = [
  "+",
  "-"
];
...
var _var = function(pairs) {
  ...
  var processedArgs = _.map(pairs, function(pair) {
  ...
  switch (tree.type) {
    ...
    case 'funcall':
      ...
      if (f.type === 'id') {
        ...
        } else if (_.include(BINARY_OR_UNIARY_OPERATORS, f.contents)) {
          if (args.length === 2) {
            return handleBinary(f, args);
          } else if (args.length === 1) {
            return handleUniary(f, args);
          } else {
            return handleMultiBinary(f, args);
          }
        ...
        } else if (f.contents === 'cond') {
          var mainList = [];
          var currentList = mainList;
          var listLen = args.length;

          if (listLen === 0) {
            throw new Error("Cond must have at least one argument!");
          }

          _.each(args, function(pair, index) {
            var condition = progn(pair.function);
            var body = processBody(pair.arguments.contents);

            if (condition.length === 2 && condition[0] === 'name' && condition[1] === 'else') {
              if (listLen === index + 1) {
                currentList.push('block', body);
              } else {
                throw new Error("else must be last argument in a cond!");
              }
            } else {
              currentList.push('if', condition, ['block', body]);
