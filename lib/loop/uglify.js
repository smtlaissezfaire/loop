    ...
    case 'funcall':
      ...
      if (f.type === 'id') {
        if (f.contents === 'lambda' || f.contents === 'function') {
          ...
          body = _.map(car(cdr(args)).contents, function(statement) {
          ...
          if (condition.type === 'funcall') {
            if (condition.arguments.contents.length > 0) {
              throw new Error('More than one statement in if conditional.');
            }

            condition = progn(condition.function);
          } else {
            condition = progn(condition);
          }


          body = _.map(args, function(statement) {
            return stmt(progn(statement));
          });
