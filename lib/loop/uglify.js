    ...
    case 'funcall':
      ...
      if (f.type === 'id') {
        if (f.contents === 'lambda') {
          ...
          throw new Error('User defined functions not yet implemented.  FIXME!');
        }
      } else if (f.type === 'funcall') {
        return stmt(['call', processStatement(tree.function), processArguments(args)]);
      } else {
        throw new Error('not a function or id in function position');
      }
    case 'id':
