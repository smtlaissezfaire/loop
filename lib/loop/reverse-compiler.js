...
var syntaxTreeToString = function(statement, formatter, options) {
  switch (statement.type) {
    case 'list':
      ...
      if (!written) {
        var lastIndex = _.lastIndexOf(contents);
...
reverseCompiler.compile = function(jsSource, options) {
  ...
  _.each(statements, function(statement, index) {
    ...
    syntaxTreeToString(statement, codeFormatter, options);

    if (!last) {
      console.log('got heree, index === ', index, 'statement.length:', statements.length);
