...
var CodeFormatter = require('./util/code-formatter').codeFormatter;
...
var extractBlockStatements = function(arg) {
  return arg[0] === 'block' ? arg[1] : [arg];
};
...
var generateIfStatement = function(args) {
  ...
  var blockStatements = extractBlockStatements(args[1]);
  ...
  if (statement instanceof Array) {
    ...
    switch (functionName) {
      ...
      case 'while':
        ...
        body = extractBlockStatements(args[0]);
        pushStatements(body, expr);
      ...
      case 'for-in':
        ...
        body = args.slice(3)[0];
        body = extractBlockStatements(body);
        pushStatements(body, expr);
...
syntaxTreeToString = function(statement, formatter, options) {
  switch (statement.type) {
        ...
        switch(id.contents) {
          ...
          case '=':
            if (options.defineIndentsLambda) {
              var secondArgList = contents[2] && contents[2].type === 'list';
              if (secondArgList) {
                usesCustomFormatter = true;
          ...
          case 'case':
            ...
            break;
          case 'label':
            if (options.labelIndentation) {
              usesCustomFormatter = true;
              indentAfterArgs(2, contents, formatter, options);
            }
            break;
          case 'for-in':
            if (options.forInIndentation) {
              usesCustomFormatter = true;
              indentAfterArgs(3, contents, formatter, options);
            }
            break;
          case 'default':
    ...
    case 'number':
      ...
      break;
    case 'comment':
      formatter.append("; " + statement.contents.trim());
      break;
    default:
...
reverseCompiler.compile = function(jsSource, options, filename, throwErrors) {
  if (!options) {
    options = {};
  }
  ...
  var parsedJsSource;
  try {
    parsedJsSource = jsp.parse(jsSource);
  } catch (e) {
    if (e.message) {
      console.error("Error parsing javascript source from " + filename + ":" + e.line + ":" + e.col);
