...
var generateIfStatement = function(args) {
  ...
  if (elseClause) {
    ...
    _.each(elseStatements, function(stmt) {
      expr.contents.push(createSyntaxTree(stmt));
    });
  }
...
var generateCond = function(args) {
  ...
  _.each(exprParts.if.body, function(stmt) {
    ifExpr.contents.push(createSyntaxTree(stmt));
  });
  ...
  var elseIfStatements = _.map(exprParts.elseIfs, function(elseIf) {
    ...
    _.each(elseIf.body, function(stmt) {
      elseIfStatement.contents.push(createSyntaxTree(stmt));
    });
  ...
  if (exprParts.else) {
    ...
    _.each(exprParts.else.body, function(stmt) {
      elseExpr.contents.push(createSyntaxTree(stmt));
    });
  }
    ...
    switch (functionName) {
      ...
      case 'try':
        ...
        if (!_.isEmpty(args)) {
          ...
          _.each(catchBody, function(stmt) {
            catchStmt.contents.push(createSyntaxTree(stmt));
          });
...
var syntaxTreeToString = function(statement, formatter, options) {
            ...
            if (options.defineIndentsLambda) {
              ...
              if (secondArgLambda) {
                ...
                formatter.indent(function() {
                  syntaxTreeToString(contents[0], formatter, options);
                });
